...
import {
  Heart, MessageCircle, ArrowLeft, Users, ChevronRight, ChevronDown,
  LayoutGrid, Clock, CheckCircle, Copy,
  Eye, UserCheck, Gamepad2, X, MapPin, RefreshCw, Info, BookOpen,
  BarChart3, Trophy, Lock, XCircle, QrCode, AlertTriangle, ShieldAlert, Maximize2, Sparkles, Camera,
} from 'lucide-react';
...
import type {
  ...
  NickTpl, UserGameSubTab, LadderBar, TutorialSlide,
} from './types/app';
export type { GameState } from './types/app';
import { hasBannedWord, koreanMatch, getMbtiStyle } from './lib/utils';
...
function ProfileQrModal({ profileId, pinCode: pinCodeProp, onClose, onPinGenerated }: {
  ...
  useEffect(() => {
    ...
    QRCode.toCanvas(contactCanvasRef.current, shareUrl, {
      width: 180, margin: 2, color: { dark: '#7c3aed', light: '#faf5ff' },
    });
  }, [tab, shareUrl]);
  ...
  useEffect(() => {
    ...
    const color = expandedTab === 'contact'
      ? { dark: '#7c3aed', light: '#faf5ff' }
      : { dark: '#0f172a', light: '#ffffff' };
    QRCode.toCanvas(largeCanvasRef.current, content, {
      width: Math.min(window.innerWidth - 80, 280), margin: 2, color,
    });
    ...
    <>
        ...
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xs overflow-hidden" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex border-b border-gray-100">
            <button
              ...
              className={`flex-1 py-3.5 text-xs font-black transition-all ${tab === 'profile' ? 'text-cyan-600 border-b-2 border-cyan-500 bg-cyan-50/60' : 'text-gray-400 hover:text-gray-600'}`}>
              🪪 프로필 QR
            </button>
            <button
              onClick={() => setTab('contact')}
              className={`flex-1 py-3.5 text-xs font-black transition-all ${tab === 'contact' ? 'text-violet-600 border-b-2 border-violet-500 bg-violet-50/60' : 'text-gray-400 hover:text-gray-600'}`}>
              📱 연락처 QR
            </button>
          </div>
          ...
          <div className="p-5 text-center">
            {tab === 'profile' ? (
              <>
                ...
                <button className="w-full relative flex justify-center mb-1.5 p-3 bg-gray-50 rounded-2xl border-2 border-dashed border-cyan-300 hover:border-cyan-500 active:scale-95 transition-all group"
                  onClick={() => { setExpandedTab('profile'); setExpanded(true); }}>
              ...
              <>
                ...
                <p className="text-xs text-gray-500 mb-3 leading-relaxed">
                  상대방이 이 QR을 스캔하면<br />
                  <strong className="text-violet-600">내 연락처가 채팅으로 자동 전달</strong>됩니다
                </p>
                <button className="w-full relative flex justify-center mb-1.5 p-3 bg-violet-50 rounded-2xl border-2 border-dashed border-violet-300 hover:border-violet-500 active:scale-95 transition-all group"
                  onClick={() => { setExpandedTab('contact'); setExpanded(true); }}>
                ...
                <div className="mt-2 mb-3 bg-violet-50 rounded-xl px-3 py-2 text-left">
                  ...
                  <p className="text-[10px] text-gray-500 leading-relaxed">
                    ① 이 QR을 상대방에게 보여주세요<br />
                    ② 상대방이 카메라 앱으로 스캔하면<br />
                    ③ 채팅방에 내 연락처가 자동으로 전송돼요
                  </p>
                </div>
              </>
            )}
            <button onClick={onClose}
              className={`w-full py-3 text-white font-bold rounded-xl transition-all ${tab === 'contact' ? 'bg-gradient-to-r from-violet-500 to-purple-600 hover:from-violet-600 hover:to-purple-700' : 'bg-gradient-to-r from-cyan-500 to-teal-500 hover:from-cyan-600 hover:to-teal-600'}`}>
              확인
            </button>
          </div>
        </div>
      ...
      {expanded && (
        <div className="fixed inset-0 z-[300] flex flex-col items-center justify-center bg-black/90 backdrop-blur-sm" onClick={() => setExpanded(false)}>
          ...
          {expandedTab === 'contact' && (
            <p className="mt-4 text-white/60 text-xs text-center">스캔 시 내 연락처가 채팅으로 자동 전달</p>
          )}
