...
export function InterestPicker({
  ...
  const renderCategory = (cat: (typeof BIO_CATEGORIES)[number]) => {
    ...
    return (
      <div key={cat.label} className={`rounded-xl border px-2.5 py-2 ${darkMode ? 'border-slate-600/70 bg-slate-800/45' : `${cat.color.border} bg-white shadow-sm shadow-gray-100/50`}`}>
        <div className="flex items-center justify-between gap-1 mb-1.5">
          ...
          {count > 0 && <span className={`text-[9px] font-bold tabular-nums ${darkMode ? 'text-cyan-400' : 'text-teal-600'}`}>{count}개</span>}
        </div>
  ...
  return (
    <div className="space-y-2.5">
      {selected.length > 0 && (
        <div className={`flex flex-wrap gap-1.5 p-2.5 rounded-2xl border ${darkMode ? 'bg-slate-800/70 border-slate-600' : 'bg-white border-cyan-100'}`}>
          ...
          {selected.map((tag) => {
            ...
            return (
              <button key={tag} type="button" onClick={() => onToggle(tag)} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-black border transition-all active:scale-95" style={{ background: style.bg, color: style.text, borderColor: style.border }}>
      ...
      <div className={`flex flex-wrap gap-1.5 rounded-xl border p-1.5 ${darkMode ? 'border-slate-600 bg-slate-800/50' : 'border-gray-200 bg-gray-50/80'}`} aria-label="관심사 대분류 선택">
        {BIO_CATEGORY_GROUPS.map((group) => {
          ...
          return (
            <button key={group.label} type="button" aria-pressed={active} onClick={() => selectGroup(group.label)} className={`inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-[11px] font-black border transition-all ${active ? 'bg-cyan-500 text-white border-cyan-500 shadow-sm' : darkMode ? 'bg-slate-800 border-slate-600 text-slate-300' : 'bg-white border-gray-200 text-gray-600 hover:border-cyan-300'}`}>
      ...
      <div className={`rounded-2xl border overflow-hidden ${darkMode ? 'border-slate-600 bg-slate-800/40' : 'border-cyan-100 bg-white'}`}>
        <div className={`flex items-center justify-between px-3.5 py-2 ${darkMode ? 'bg-slate-800' : 'bg-cyan-50'}`}>
          <span className="text-[11px] font-black text-cyan-700">{GROUP_EMOJI[activeGroup.label] ?? ''} {activeGroup.label}</span>
          <span className={`text-[10px] font-bold ${darkMode ? 'text-slate-500' : 'text-gray-400'}`}>대분류 선택 · 소분류 태그</span>
        </div>
