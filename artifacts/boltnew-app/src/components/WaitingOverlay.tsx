        ...
        <div className="w-full rounded-2xl bg-amber-500/15 border border-amber-400/30 px-4 py-3 mb-2">
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-amber-200 font-black text-xs leading-snug">이미 프로필을 만드셨나요?</p>
              <p className="text-amber-300/80 text-[11px] leading-snug">
                <span className="font-bold text-amber-200">고유번호(PIN)</span>로 입장하세요
              </p>
            </div>
            <button
              onClick={() => { setShowPinRecovery(true); setPinDigits(['','','','']); setPinError(''); setPinStep('pin'); }}
              className="flex-shrink-0 flex items-center gap-1 px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 active:scale-95 transition-all text-white font-bold text-xs shadow whitespace-nowrap"
            >
              🔑 PIN 입장
            </button>
          </div>
        </div>
        {/* 고유번호 복구 (기존 오렌지 버튼 유지) */}
        <button
          onClick={() => { setShowPinRecovery(true); setPinDigits(['','','','']); setPinError(''); setPinStep('pin'); }}
