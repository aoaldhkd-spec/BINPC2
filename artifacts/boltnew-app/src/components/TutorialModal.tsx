...
function TipCard({ tip, panel, text, muted }: {
  ...
  return (
    <div className={`flex gap-1.5 rounded-lg border px-1.5 py-1 ${panel}`}>
      <span className="text-[12px] leading-none mt-px flex-shrink-0">{tip.icon}</span>
      <div className="min-w-0">
        <p className={`text-[10px] font-black leading-tight ${text}`}>{tip.title}</p>
        <p className={`text-[9px] leading-snug mt-px ${muted}`}>{tip.desc}</p>
      </div>
    </div>
  );
}
...
function TipGrid({ tips, panel, text, muted }: {
  ...
  return (
    <div className={`grid gap-1 ${tips.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
      {tips.map((tip) => (
            ...
            <div className="flex-shrink-0 flex flex-col gap-2">
              {(topic.sections ?? []).map((section) => (
                <div key={section.title}>
                  <p className={`text-[10px] font-black mb-1 ${text}`}>{section.emoji} {section.title}</p>
