...
function SlideWithSteps({ desc, steps, darkMode }: { desc: string; steps: string[]; darkMode?: boolean }) {
  return (
    <div className="px-5 py-5 space-y-4">
      ...
      <div className={`rounded-2xl p-4 space-y-3 ${darkMode ? 'bg-slate-800/80 border border-slate-700' : 'bg-slate-50 border border-slate-200'}`}>
        <p className={`text-[9px] font-black uppercase tracking-widest ${darkMode ? 'text-slate-500' : 'text-gray-400'}`}>📌 이렇게 사용해요</p>
        {steps.map((step, i) => (
          <div key={i} className="flex items-start gap-3">
            <span className="flex-shrink-0 w-5 h-5 rounded-full bg-cyan-500 text-white text-[10px] font-black flex items-center justify-center mt-0.5 leading-none">{i + 1}</span>
            <p className={`text-[12px] leading-snug ${darkMode ? 'text-slate-300' : 'text-gray-600'}`}>{step}</p>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="px-5 pt-5 pb-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2.5">
          {([
            ...
            <div key={label} className={`flex items-center gap-3 px-3.5 py-3 rounded-2xl ${darkMode ? 'bg-slate-800 border border-slate-700' : 'bg-gradient-to-br from-cyan-50 to-teal-50 border border-cyan-100'}`}>
              <span className="text-xl leading-none flex-shrink-0">{icon}</span>
              <div>
                <p className={`text-[12px] font-black leading-tight ${darkMode ? 'text-slate-200' : 'text-cyan-900'}`}>{label}</p>
                <p className={`text-[10px] leading-snug mt-0.5 ${darkMode ? 'text-slate-500' : 'text-cyan-600'}`}>{sub}</p>
              </div>
            </div>
          ))}
        </div>
        <p className={`text-[11px] text-center font-medium ${darkMode ? 'text-slate-500' : 'text-gray-400'}`}>
          👇 탭을 눌러 각 기능을 확인하세요
        </p>
      </div>
    ),
  ...
  /* 1 — 공지사항 */
  {
    ...
    renderBody: (darkMode) => (
      <div className="px-5 py-5">
        <div className={`rounded-2xl p-4 space-y-2.5 ${darkMode ? 'bg-slate-800 border border-slate-700' : 'bg-teal-50 border border-teal-100'}`}>
          {[
            ...
            '모든 저작권은 범일NPC에 있습니다. 불법 복제·도용 시 민형사상 책임',
          ].map((text, i) => (
            <div key={i} className="flex items-start gap-3">
              <span className={`flex-shrink-0 w-5 h-5 rounded-full text-[10px] font-black flex items-center justify-center mt-0.5 leading-none ${darkMode ? 'bg-teal-700 text-teal-200' : 'bg-teal-500 text-white'}`}>{i + 1}</span>
              <p className={`text-[12px] leading-snug ${darkMode ? 'text-slate-300' : 'text-teal-900'}`}>{text}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  /* 2 — 하트 */
  {
    ...
    renderBody: (darkMode) => (
      <SlideWithSteps darkMode={darkMode}
        desc="참여자 탭에서 마음에 드는 분에게 하트를 보내세요. 종류별 2개씩 총 8개를 사용할 수 있어요."
        steps={[
          '참여자 탭 → 카드 오른쪽 위 🤍 버튼 터치',
          '하트 종류 선택\n❤️ 맘에 들어요  💙 친구해요  🧡 뜨밤  💚 칭찬해요',
          '❤️💙🧡 수락 시 연락처 교환 & 채팅 오픈!',
          '💚 칭찬 하트는 수락 즉시 확인 가능 (채팅 없음)',
        ]}
      />
    ),
  },
  ...
  /* 5 — 숨겨진 기능 TIP */
  {
    ...
    renderBody: (darkMode) => (
      <div className="px-5 py-5 space-y-3">
        <p className={`text-[12px] leading-relaxed ${darkMode ? 'text-slate-300' : 'text-gray-500'}`}>
        ...
        <div className="space-y-2.5">
          {([
            ...
            <div key={title} className={`flex items-start gap-3 px-4 py-3 rounded-2xl ${darkMode ? 'bg-slate-800 border border-slate-700' : 'bg-violet-50 border border-violet-100'}`}>
              ...
              <div>
                ...
                <p className={`text-[11px] leading-snug mt-1 ${darkMode ? 'text-slate-400' : 'text-violet-600/80'}`}>{desc}</p>
              </div>
            </div>
          ))}
        </div>
...
export function TutorialModal({ page, onChangePage, onClose, darkMode }: {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-5 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div
        className={`relative w-full max-w-sm rounded-3xl shadow-2xl flex flex-col overflow-hidden transition-all ${darkMode ? 'bg-slate-900' : 'bg-white'}`}
      ...
      >
        ...
        <div className={`bg-gradient-to-br ${slide.color} px-6 py-5 text-center flex flex-col items-center justify-center flex-shrink-0`}>
          <div className="text-3xl mb-2">{slide.emoji}</div>
        ...
        <div className="flex-1">
          {slide.renderBody ? slide.renderBody(darkMode) : (
            <div className={`px-5 py-5 text-[12px] leading-relaxed whitespace-pre-line ${darkMode ? 'text-slate-300' : 'text-gray-600'}`}>
              {slide.desc}
            </div>
          )}
        </div>
