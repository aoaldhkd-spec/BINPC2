...
App residual is compose/wiring (setState fan-in, guarded handlers, overlay props). **db.ts peel progressing:** prior modules + **auth-resolve/login-body + unread rejects/cache + push-notify validate + leave-expand/opt-out + sse-gate/notify-queue/counts + admin-NPC store patch + broadcast rejects/IP + clear-db-errors auth** folded into pure libs (re-import; single router export intact). Full `/op`/RPC router split remains the large mountain (~5.3k).
...
| This peel (auth-resolve + unread/push-notify + leave-expand + sse-gate) | **~9.5** | auth userId resolve/login body + unread rejects/cache + push-notify validate + group leave-expand/opt-out + SSE token gate/notify-queue/counts + admin-NPC store patch + broadcast rejects/IP + clear-db-errors auth folded into existing pure libs; `db.ts` ~5.30k. Honest claim still **~9.5** — further gains are write-path / `/op` slices |
