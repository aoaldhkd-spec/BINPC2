...
App residual is compose/wiring (setState fan-in, guarded handlers, overlay props). **db.ts peel progressing:** prior modules + **overlay-secrets + pin-collect + integrity-clamp + dedupe/merge-apply + critical-write-log** folded into existing pure libs (re-import; single router export intact). Full `/op`/RPC router split remains the large mountain (~5.18k).
...
| This peel (overlay-secrets + pin-collect + integrity-clamp + dedupe/merge-apply + critical-write-log) | **~9.5** | Settings secret overlay + used-PIN collect + integrity env clamps + chat_reads/group-participant memory apply + critical-write log helper folded into existing pure libs; `db.ts` ~5.18k. Honest claim still **~9.5** — further gains are write-path / `/op` slices |
