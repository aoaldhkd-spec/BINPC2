...
App residual is compose/wiring (setState fan-in, guarded handlers, overlay props). **db.ts peel progressing:** prior modules + **`db-group-room-plan`** + **`db-chat-pair-plan`** (re-import; single router export intact). Full `/op`/RPC router split remains the large mountain (~6.0k).
...
| This peel (db-chat-pair-plan) | **~9.5** | Chat pair / dedupe / message-merge pure planners peeled; `db.ts` still ~6.0k. Honest claim still **~9.5** — further gains are write-path/ensureAdmin |
...
2. Continue `db.ts` peel track (ensureAdmin/write-path / rate-bucket clusters + re-import); keep `38_db_single_router_export`.
