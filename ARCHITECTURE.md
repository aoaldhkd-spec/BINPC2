...
App residual is compose/wiring (setState fan-in, guarded handlers, overlay props). **db.ts peel progressing:** prior modules + **chat-dedupe/message-merge + group-merge/catalog specs + entry-password renewal** folded into existing pure libs (re-import; single router export intact). Full `/op`/RPC router split remains the large mountain (~5.25k).
...
| This peel (chat-dedupe + group-merge + entry-renewal) | **~9.5** | 1:1 chat dedupe/read-merge + message apply + group participant merge/catalog specs + daily entry-password renewal folded into existing pure libs; `db.ts` ~5.25k. Honest claim still **~9.5** — further gains are write-path / `/op` slices |
