...
- `heart_grants`는 선택적인 고급 표시용 필드로 유지되지만, 참여자 전송은 `rainbow_pool`이 열린 뒤에만 가능하며 서버는 색상과 무관하게 pool의 총 사용량으로 한도를 검증한다.
- 관리자 **해금**은 해당 슬롯 시각을 현재 Asia/Seoul 시각으로 맞추고 `functions_locked=false`와 `rainbow_pool`을 저장·방송한다. **지금 적용**은 pool 없이 슬롯을 지금 열고 동일하게 즉시 저장·방송한다. 참여자 공지 배너는 슬롯 공지 구간(다음 슬롯 또는 5분)이 지나면 자동으로 숨긴다.
